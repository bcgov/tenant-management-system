...
describe('Tenant API', () => {
  ...
  describe('POST /v1/tenant-requests', () => {
    ...
    it('should return 500 when repository throws an error', async () => {
      mockTMSRepository.saveTenantRequest.mockRejectedValue(
        new Error('Database error'),
      )
  ...
  describe('GET /v1/tenant-requests', () => {
    ...
    it('should get all tenant requests successfully', async () => {
      ...
      const response = await request(app).get('/v1/tenant-requests')
    ...
    it('should get tenant requests filtered by NEW status', async () => {
      ...
      const response = await request(app).get('/v1/tenant-requests?status=NEW')
    ...
    it('should return 400 when status parameter is invalid', async () => {
      const response = await request(app).get(
        '/v1/tenant-requests?status=INVALID_STATUS',
      )
  ...
  describe('PUT /v1/shared-services/:sharedServiceId/shared-service-roles/:sharedServiceRoleId', () => {
    const sharedServiceId = '123e4567-e89b-12d3-a456-426614174000'
    const sharedServiceRoleId = '123e4567-e89b-12d3-a456-426614174001'
    const validUpdateData = {
      name: 'Updated Role',
    ...
    beforeEach(() => {
      app.put(
        ...
        (req, res) => tmsController.updateSharedServiceRole(req, res),
      )
    ...
    it('should update a shared service role successfully', async () => {
      const mockUpdatedSharedService = {
        id: sharedServiceId,
        name: 'Test Shared Service',
        clientIdentifier: 'test-service-client',
        ...
        roles: [
          {
            ...
            name: validUpdateData.name,
            description: validUpdateData.description,
            allowedIdentityProviders: validUpdateData.allowedIdentityProviders,
            isDeleted: false,
          },
        ],
        createdDateTime: new Date(),
      ...
      mockTMSRepository.updateSharedServiceRole.mockResolvedValue(
        mockUpdatedSharedService,
      )
            ...
            roles: expect.arrayContaining([
              expect.objectContaining({
                ...
                allowedIdentityProviders:
                  validUpdateData.allowedIdentityProviders,
              }),
            ]),
          },
        },
      ...
      expect(mockTMSRepository.updateSharedServiceRole).toHaveBeenCalledWith(
        expect.objectContaining({
          sharedServiceId,
          sharedServiceRoleId,
          ...validUpdateData,
    ...
    it('should allow clearing description and identity providers', async () => {
      const mockUpdatedSharedService = {
        id: sharedServiceId,
        name: 'Test Shared Service',
        clientIdentifier: 'test-service-client',
      ...
      expect(response.status).toBe(200)
      expect(response.body.data.sharedService.roles[0]).toMatchObject({
        description: null,
        allowedIdentityProviders: null,
      })
      expect(mockTMSRepository.updateSharedServiceRole).toHaveBeenCalledWith(
    ...
    it('should return 404 when shared service role is not found', async () => {
      const errorMessage = `Shared service role not found: ${sharedServiceRoleId}`
      mockTMSRepository.updateSharedServiceRole.mockRejectedValue(
        new NotFoundError(errorMessage),
      )
      ...
      expect(response.body).toMatchObject({
        ...
        httpResponseCode: 404,
        message: errorMessage,
        name: 'Error occurred updating shared service role',
      })
    })

    it('should return 409 when role name already exists', async () => {
      const errorMessage =
        "Role 'Updated Role' already exists for this shared service"
      mockTMSRepository.updateSharedServiceRole.mockRejectedValue(
        new ConflictError(errorMessage),
      )
    ...
    it('should return 400 when shared service role id is invalid', async () => {
      const response = await request(app)
        .put(
          `/v1/shared-services/${sharedServiceId}/shared-service-roles/invalid-role-id`,
        )
        .send(validUpdateData)
  ...
  describe('PATCH /v1/shared-services/:sharedServiceId/status', () => {
    const sharedServiceId = '123e4567-e89b-12d3-a456-426614174000'
    ...
    beforeEach(() => {
      app.patch(
        ...
        (req, res) => tmsController.updateSharedServiceStatus(req, res),
      )
    ...
    it('should disable a shared service successfully', async () => {
      const mockUpdatedSharedService = {
        id: sharedServiceId,
        name: 'Test Shared Service',
        displayName: 'Test Shared Service Display',
      ...
      mockTMSRepository.updateSharedServiceStatus.mockResolvedValue(
        mockUpdatedSharedService,
      )

      const response = await request(app)
        .patch(`/v1/shared-services/${sharedServiceId}/status`)
        .send({ isActive: false })
      ...
      expect(mockTMSRepository.updateSharedServiceStatus).toHaveBeenCalledWith(
        expect.objectContaining({
          sharedServiceId,
          isActive: false,
          updatedBy: 'system',
    ...
    it('should enable a shared service successfully', async () => {
      const mockUpdatedSharedService = {
        id: sharedServiceId,
        name: 'Test Shared Service',
        displayName: 'Test Shared Service Display',
      ...
      expect(response.status).toBe(200)
      expect(response.body.data.sharedService.isActive).toBe(true)
    })

    it('should return 404 when shared service is not found', async () => {
      const errorMessage = `Shared service not found: ${sharedServiceId}`
      mockTMSRepository.updateSharedServiceStatus.mockRejectedValue(
        new NotFoundError(errorMessage),
      )
      ...
      expect(response.body).toMatchObject({
        ...
        httpResponseCode: 404,
        message: errorMessage,
        name: 'Error occurred updating shared service status',
      })
    })

    it('should return 409 when shared service is already inactive', async () => {
      mockTMSRepository.updateSharedServiceStatus.mockRejectedValue(
        new ConflictError('Shared service is already inactive'),
      )
      ...
      expect(response.body).toMatchObject({
        errorMessage: 'Conflict',
        httpResponseCode: 409,
        message: 'Shared service is already inactive',
        name: 'Error occurred updating shared service status',
      })
    })

    it('should return 409 when shared service is already active', async () => {
      mockTMSRepository.updateSharedServiceStatus.mockRejectedValue(
        new ConflictError('Shared service is already active'),
      )
      ...
      expect(response.body).toMatchObject({
        errorMessage: 'Conflict',
        httpResponseCode: 409,
        message: 'Shared service is already active',
        name: 'Error occurred updating shared service status',
      })
    })
    ...
    it('should return 400 when shared service id is invalid', async () => {
      const response = await request(app)
        .patch('/v1/shared-services/invalid-shared-service-id/status')
        .send({ isActive: false })
  ...
  describe('GET /v1/shared-services', () => {
    beforeEach(() => {
      app.get('/v1/shared-services', (req, res) =>
    ...
    it('should get all active shared services successfully', async () => {
      ...
      const response = await request(app).get('/v1/shared-services')
      ...
      expect(
        mockTMSRepository.getAllActiveSharedServices,
    ...
    it('should exclude deleted roles from shared services', async () => {
      ...
      expect(response.body.data.sharedServices[0].roles[0].isDeleted).toBe(
        false,
      )
    })
  ...
  describe('POST /v1/tenants/:tenantId/shared-services', () => {
    ...
    it('should associate shared service to tenant successfully', async () => {
      mockTMSRepository.associateSharedServiceToTenant.mockResolvedValue(
        undefined,
      )
    ...
    it('should fail when tenant not found', async () => {
      ...
      const response = await request(app)
        .post(
          '/v1/tenants/123e4567-e89b-12d3-a456-426614174999/shared-services',
        )
        .send(validRequestData)
    ...
    it('should fail when shared service is inactive', async () => {
      mockTMSRepository.associateSharedServiceToTenant.mockRejectedValue(
        new ConflictError(
          "Cannot associate inactive shared service 'Inactive Service' to tenant",
        ),
      )
    ...
    it('should fail when shared service already associated', async () => {
      mockTMSRepository.associateSharedServiceToTenant.mockRejectedValue(
        new ConflictError(
          "Shared service 'Test Service' is already associated with this tenant",
        ),
      )
  ...
  describe('GET /v1/tenants/:tenantId/shared-services', () => {
    ...
    it('should return 400 when tenant ID is invalid', async () => {
      const response = await request(app).get(
        '/v1/tenants/invalid-tenant-id/shared-services',
      )
